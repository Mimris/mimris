...
export function buildGoModelviewTypesModel(
  metamodel: akm.cxMetaModel,
  modelview: akm.cxModelView,
  includeDeleted: boolean,
  showModified: boolean,
): gjs.goModel | undefined {
  if (!metamodel || !modelview) return;
  ...
  if (!objecttypeviews.length && !relshiptypeviews.length) return;

  const myGoModel = new gjs.goModel(modelview.id || utils.createGuid(), "myTypeModelview", modelview);
  const includedObjecttypeIds = new Set<string>();

  for (let i = 0; i < objecttypeviews.length; i++) {
    const typeview = objecttypeviews[i] as akm.cxObjectTypeView;
    const objtypeRef = typeview?.typeRef || typeview?.objecttypeRef || typeview?.objtypeRef;
    const objtype =
      (objtypeRef && metamodel.findObjectType?.(objtypeRef)) ||
      (typeview?.name && metamodel.findObjectTypeByName?.(typeview.name));
    if (!objtype) continue;
    if (objtype.markedAsDeleted && !includeDeleted) continue;

    const previousTypeview = objtype.typeview;
    objtype.typeview = typeview;
    const node = new gjs.goObjectTypeNode(typeview.id || objtype.id || utils.createGuid(), objtype);
    if (node.loadNodeContent(metamodel)) {
      if (showModified && objtype.modified) node.strokecolor = "green";
      myGoModel.addNode(node);
      if (objtype.id) includedObjecttypeIds.add(objtype.id);
    }
    objtype.typeview = previousTypeview;
  }

  for (let i = 0; i < relshiptypeviews.length; i++) {
    const typeview = relshiptypeviews[i] as akm.cxRelationshipTypeView;
    const reltypeRef = typeview?.typeRef || typeview?.relshiptypeRef || typeview?.reltypeRef;
    const reltype =
      (reltypeRef && metamodel.findRelationshipType?.(reltypeRef)) ||
      (reltypeRef && metamodel.findRelshipType?.(reltypeRef)) ||
      (typeview?.name && metamodel.findRelationshipTypeByName?.(typeview.name)) ||
      (typeview?.name && metamodel.findRelshipTypeByName?.(typeview.name));
    if (!reltype) continue;
    if (reltype.markedAsDeleted && !includeDeleted) continue;
    ...
    const fromObjtype = reltype.fromObjtype || metamodel.findObjectType?.(reltype.fromobjtypeRef);
    const toObjtype = reltype.toObjtype || metamodel.findObjectType?.(reltype.toobjtypeRef);
    if (!fromObjtype || !toObjtype) continue;
    if (!includedObjecttypeIds.has(fromObjtype.id) || !includedObjecttypeIds.has(toObjtype.id)) continue;

    const previousTypeview = reltype.typeview;
    reltype.typeview = typeview;
    const link = new gjs.goRelshipTypeLink(typeview.id || reltype.id || utils.createGuid(), myGoModel, reltype);
    link.fromNode = myGoModel.findTypeNode(fromObjtype.id);
    link.toNode = myGoModel.findTypeNode(toObjtype.id);
    link.from = link.fromNode?.key ?? fromObjtype.id;
    link.to = link.toNode?.key ?? toObjtype.id;
    if (link.loadLinkContent()) {
      myGoModel.addLink(link);
    }
    reltype.typeview = previousTypeview;
  }
