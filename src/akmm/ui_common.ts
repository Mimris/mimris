...
export function createObject(data: any, context: any): akm.cxObjectView | null {
    if (data === null) {
        ...
        if (!objtype)
            return null;
        console.log('31 createObject', myMetis, data);
        let obj = data.object;
        if (!myModel.pasteViewsOnly) {
            ...
            if (obj) {
                ...
                console.log('39 createObject', obj, myModel, myGoModel);
            }
        }
...
export function deleteNode(data: any, deletedFlag: boolean, deletedNodes: any, deletedObjects: any, deletedTypeviews: any, context: any) {
    const myMetis     = context.myMetis;
    const myMetamodel = context.myMetamodel;
    if (data.category === constants.gojs.C_OBJECTTYPE) {
...
export function createRelationship(data: any, context: any) {
    ...
    if (!reltype) {
        ...
        if ((myMetis) && (fromType && toType)) {
            ...
            console.log('715 createRelationship', reltypes, myMetis);
            if (reltypes) {
...
export function pasteRelationship(data: any, nodes: any[], context: any) {
    ...
    console.log('762 myMetis', myMetis, myGoModel);
