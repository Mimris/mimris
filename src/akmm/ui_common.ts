        ...
        if (fromType && toType) {
            ...
            if (reltypes) {
                ...
                if (!selectRelshipTypesUsingModal) {    // set to false when using a modal dialog
                    ...
                    if (!debug) console.log('1042 data, myDiagram', data, myDiagram);
                    // myDiagram.model.setDataProperty(data, "name", typename);
...
export function createRelshipCallback(args:any): akm.cxRelationshipView {
    if (!debug) console.log('1048 args', args);
    ...
    if (debug) console.log('1059 reltype', reltype);
    if (!reltype) {
        alert("Relationship type given does not exist!")
        myDiagram.model.removeLinkData(data);
        return;
    }
    if (!debug) console.log('1072 typename, data, reltype', typename, data, reltype);
   if (selectRelshipTypesUsingModal) {
    ...
    if (!debug) console.log('1092 data, relshipview', data, relshipview);
    if (relshipview) {
        ...
        if (debug) console.log('1105 relshipview', relshipview);
    ...
    if (reltype) {
        ...
        if (currentRelship) {
            ...
            if (currentRelshipView) {
                ...
                if (debug) console.log('1217 SetReltype', gqlRelView);
...
export function addRelationshipViewsToObjectView(modelview: akm.cxModelView, objview: akm.cxObjectView, myMetis: akm.cxMetis) {
    ...
    if (outrels && inrels)
        ...
        rels = outrels;
    if (debug) console.log('1648 rels', rels);
    for (let j=0; j<rels?.length; j++) {
        ...
        const rviews = rel.relshipviews;
        const mrelviews = modelview.relshipviews;
        if (debug) console.log('1657 rviews, mrelviews', rviews, mrelviews)
        let found = false;
        if (rviews?.length > 0) {
            for (let i=0; i<rviews.length; i++) {
            const rv = rviews[i];
        ...
        if (debug) console.log('1681 fromObjviews', fromObjviews);
        const toObj = rel.toObject as akm.cxObject;
        const toObjviews = toObj.objectviews;
        if (toObjviews?.length == 0) {
          // From objview is NOT in modelview - do nothing
        ...
        if (debug) console.log('1688 rel', rel);
        // Relview(s) does not exist, but from and to objviews exist, create relview(s)
        const relview = new akm.cxRelationshipView(utils.createGuid(), rel.name, rel, rel.description);
        if (relview.markedAsDeleted) continue;
        let fromObjview = null;
        ...
        if (fromObjview && toObjview) {
            ...
            if (debug) console.log('1706 relview', relview);
            modelview.addRelationshipView(relview);
            // Add link
            const myGoModel = myMetis.gojsModel;
            let link = new gjs.goRelshipLink(utils.createGuid(), myGoModel, relview);
            link.loadLinkContent(myGoModel);
            myGoModel.addLink(link);
            if (debug) console.log('1713 relview, link', relview, link);
            // Prepare and do the dispatch
            const gqlRelview = new gql.gqlRelshipView(relview);
            modifiedRelshipViews.push(gqlRelview);
    ...
    if (debug) console.log('1724 myMetis', myMetis);
}
...
export function updateLink(data: any, reltypeView: akm.cxRelationshipTypeView, diagram: any, goModel: gjs.goModel) {
    if (reltypeView) {
        if (!debug) console.log('2559 diagram', diagram);
        let viewdata: any = reltypeView.getData();
        let lnk = diagram.findLinkForKey(data.key);
        let prop: string;
        for (prop in viewdata) {
            if (prop === 'abstract') continue;
            ...
            if (relview) {
                if (relview[prop] && relview[prop] !== "") {
                    diagram.model.setDataProperty(lnk, prop, relview[prop]);
                }
                if (debug) console.log('2574 updateLink', data, prop, viewdata[prop]);
            }
        }
    }
} 
