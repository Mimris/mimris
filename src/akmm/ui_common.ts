        ...
        if (obj) {
            ...
            if (objview) {
                ...
                if (node) {
                    node.loc = data.loc;
                    node.size = data.size;
                    if (debug) console.log('89 node, data, myGoModel', node, data, myGoModel);
                    ...
                    const group = getGroupByLocation(myGoModel, node.loc, node.size, node);
...
export function addHasMemberRelship(node: any, myMetis: akm.cxMetis, myModelview: akm.cxModelView, myDiagram: any) {
    const myGoModel = myMetis.gojsModel;
...
export function getNodeByViewId(viewId: string, diagram: any): any {
    const nodes = diagram.nodes;
    for (let it = nodes.iterator; it?.next();) {
        const node = it.value;
        if (node.data.objectview.id == viewId) 
            return node;
    }
    return null;
}
