...
export function addConnectedObjects(modelview: akm.cxModelView, objview: akm.cxObjectView, goModel: gjs.goModel, 
                                    myMetis: akm.cxMetis, noLevels: number, reltypeNames: string, reldir: string) {
    if (noLevels < 1)
        return;
    const objectviews = [];
    const myDiagram = myMetis.myDiagram;
    const node = uid.getNodeByViewId(objview.id, myDiagram);
    ...
    if (noLevels > 1) {
        ...
        for (let i=0; i<objectviews?.length; i++) {
            ...
            addConnectedObjects(modelview, oview, goModel, myMetis, noLevels, reltypeNames, reldir);
        }
    }
}
...
export function traverseViews(objview: akm.cxObjectView, context: any): boolean {
    ...
    if (relviews) {
        ...
        for (let i=0; i<relviews.length; i++) {
            ...
            let toObjview;
            if (relview.toObjview?.id === objview.id) {
                toObjview = relview.fromObjview as akm.cxObjectView;
            } else if (relview.fromObjview.id === objview.id) {
