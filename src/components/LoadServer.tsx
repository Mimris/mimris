...
const SelectSource = (props: any) => {
  ...
  useEffect(() => {
    ...
    function refres() {
      selmodellist = (modellist.modList) && modellist?.modList.map(ml => (ml) &&  {value: ml.id, label: ml.name})
    }
    setTimeout(refres, 1);
  }, [])
  ...
  modellist = props.ph.phList
  
  console.log('29', props.ph.phList);

  useEffect(() => {
    selmodellist = (modellist.modList) && modellist?.modList.map(ml => (ml) &&  {value: ml.id, label: ml.name})
    console.log('33', props.ph.phList, modellist, selmodellist);
  ...
  function handleLoadModelStore() { 
    dispatch(loadDataModelList())
    console.log('63', props.ph.phList);
  }
  ...
  const selmodellistDiv =  <Select options={selmodellist} />
...
//   const [locModelId, setLocModelId] = useLocalStorage('modelId', null);
...
//     const res = await fetch('http://localhost:4000/akm-model-list',
//       {
//         mode: 'no-cors',
//         headers: {
...
//     console.log('86', list);
//     const optionstmp = list.map(l => (l) && {value: l.id, label: l.name})
//     // // const options = optionstmp.result
//     let options = [ ...optionstmp,
//       // { value: 'chocolate', label: 'Chocolate' },
//       // { value: 'strawberry', label: 'Strawberry' },
//       { value: 'vanilla', label: 'Vanilla' }
//     ]
//     console.log('93', list, optionstmp, options);
//     return options
//     // id='+locModelId)
//   }
...
//   function handleLoadModelStore() { 
//     tmp = 1
//     console.log('112', fetchModelList() );
//     const selmodels = fetchModelList()
//     console.log('114 LoadServer', selmodels);
//     return selmodels
//     setRefresh(!refresh)
//     // selectorDiv = <Select options={options} />
//     // dispatch(loadData())
...
//   let selmodels = fetchModelList()
//   // selmodels = selmodeltmp
//   console.log('88', selmodels, selmodelss);

//   let selectorDiv = (selmodels) && <Select options={selmodels} />
//   let tmp = 0
//   // console.log('101 LoadServer', selectorDiv);

//   useEffect(() => {
//     selmodels = fetchModelList()
//     console.log('102 useEffect LoadSerer', selmodels);

//     // selectorDiv = <Select options={selmodels} />
//   }, [selmodels])



  
//   console.log('120 LoadServer', selmodels);
  
//   const frameId = 'myFrame'

//   // const selectorDiv2 = (selmodels) &&
//   //   <div className="modeller-selection p-2 " >
//   //     <Selector type='SET_FOCUS_MODEL' selArray={selmodels} selName='Model' focustype='focusModel' refresh={refresh} setRefresh={setRefresh} /> <br /><hr />
//   //     {/* <Selector type='SET_FOCUS_MODELVIEW' selArray={selmodelviews} selName='Modelviews' focustype='focusModelview' refresh={refresh} setRefresh={setRefresh} />  <br /> */}
//   //   </div> 
...
//           </div>
//           {/* <Button className="modal-footer m-0 py-1 px-2" color="link" onClick={modeldata} >Load</Button> */}
//             {refresh ? <>{selectorDiv}</> : <>{selectorDiv}</> }
//         </ModalBody>
//         <ModalFooter>
//           <div style={{ fontSize: "smaller" }}>
//             NB! Clicking "Load" will overwrite current store (memory).
