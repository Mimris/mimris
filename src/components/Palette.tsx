    ...
    const palette =
      <> 
        <button className="btn-sm pt-0 pb-0 b-0 mt-0 mr-2" style={{ backgroundColor: "#999", outline: "0", borderStyle: "none"}}
          onClick={togglePalette}> {visiblePalette ? <span> &lt;  Palette Metamod </span> : <span>&gt;</span>} 
        </button>
          {visiblePalette 
            ? <div> {gojsapp} <div style={{ minWidth: "290px" }}></div></div>
            : <div className="btn-vertical m-0 pl-2 p-0" style={{ maxWidth: "4px", padding: "0px" }}><span> P a l e t t e - M e t a</span> </div>
          }
      </>  
