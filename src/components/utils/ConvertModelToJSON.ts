...
export const WriteConvertModelToJSONFile = async (model, curmodelview, mname, type) => {
    ...
    if (debug) console.log('11',  model, curmodelview);
    ...
    if (debug) console.log('14',  filteredObjects);
...
export const ConvertObjectsToJsonStructure = (objects) => {
    ...
    const genJsonObj = (objects) => {
        ...
        let JSONObject = objAll.replace(/,}/g, '}').replace(/,]/g, ']').replace(/{,/g, '{').replace(/\[,/g, '[').replace(/\{},/g, '')//.replace(/,}/g, '}}').replace(/{,/g, '{')
